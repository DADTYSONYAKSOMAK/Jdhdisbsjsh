...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@SOU_RCE_VENOM"))
...
⚝ مرحبا بك في اوامر فينوم بـوينت
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython1.send_message(event.chat_id, f"تم الانتهاء من التجميع | ᏙᎬ")
        ...
        await sython1.send_message(event.chat_id, "تم الانتهاء من التجميع | ᏙᎬ")
    ...
    while True:
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await sython1(JoinChannelRequest('saythonh'))
                channel_entity = await sython1.get_entity(pot)
                await sython1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة فينوم**')
                await sython1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await sython1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinq = await sython1(JoinChannelRequest('billionbot3'))
        joinw = await sython1(JoinChannelRequest('Fvvvv'))
        joine = await sython1(JoinChannelRequest('FFF22'))
        joinr = await sython1(JoinChannelRequest('q2qqqq'))
        joint = await sython1(JoinChannelRequest('F_1_T'))
        joiny = await sython1(JoinChannelRequest('KTTTT'))
        joini = await sython1(JoinChannelRequest('zGzzz'))
        joino = await sython1(JoinChannelRequest('iiiPi'))
        joinp = await sython1(JoinChannelRequest('KTTTTGiFT'))
        joina = await sython1(JoinChannelRequest('IIIlp'))
